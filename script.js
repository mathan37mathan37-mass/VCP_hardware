...
function validateLeadForm(type, body) {
  const rules = {
    customer: [
      { field: '#customer-name', label: 'Name', value: body.querySelector('#customer-name')?.value?.trim() || '', required: true },
      { field: '#customer-company', label: 'Company name', value: body.querySelector('#customer-company')?.value?.trim() || '', required: true },
      { field: '#customer-email', label: 'Email', value: body.querySelector('#customer-email')?.value?.trim() || '', required: true, validator: isValidEmailAddress, error: 'Enter a valid email address.' },
      { field: '#customer-phone', label: 'Phone', value: body.querySelector('#customer-phone')?.value?.trim() || '', required: true, validator: isValidPhoneNumber, error: 'Phone number must be 10 digits.' },
      { field: '#customer-project', label: 'Requirement', value: body.querySelector('#customer-project')?.value?.trim() || '', required: true }
    ],
    callback: [
      { field: '#callback-name', label: 'Name', value: body.querySelector('#callback-name')?.value?.trim() || '', required: true },
      { field: '#callback-phone', label: 'Phone', value: body.querySelector('#callback-phone')?.value?.trim() || '', required: true, validator: isValidPhoneNumber, error: 'Phone number must be 10 digits.' },
      { field: '#callback-email', label: 'Email', value: body.querySelector('#callback-email')?.value?.trim() || '', required: true, validator: isValidEmailAddress, error: 'Enter a valid email address.' },
      { field: '#callback-requirement', label: 'Requirement', value: body.querySelector('#callback-requirement')?.value?.trim() || '', required: true }
    ],
    'supply-list': [
      { field: '#supply-name', label: 'Name', value: body.querySelector('#supply-name')?.value?.trim() || '', required: true },
      { field: '#supply-email', label: 'Email', value: body.querySelector('#supply-email')?.value?.trim() || '', required: true, validator: isValidEmailAddress, error: 'Enter a valid email address.' },
      { field: '#supply-phone', label: 'Phone', value: body.querySelector('#supply-phone')?.value?.trim() || '', required: true, validator: isValidPhoneNumber, error: 'Phone number must be 10 digits.' },
      { field: '#supply-industry', label: 'Supply area', value: body.querySelector('#supply-industry')?.value?.trim() || '', required: true }
    ]
  };

  const checks = rules[type] || [];

  for (const check of checks) {
    if (check.required && !check.value) {
      return `${check.label} is required before submitting.`;
    }

    if (check.validator && check.value && !check.validator(check.value)) {
      return check.error || `${check.label} is invalid.`;
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bestSellersTrigger = document.querySelector('[data-action="best-sellers"]');
  ...
  // Initialize product visibility correctly on page load.
  ...
  const modalForms = {
    ...
    'admin-login': {
      title: 'Admin Access',
      body: `<p>Enter the admin password to open the dashboard.</p>
        <div class="modal-form-grid">
          <div class="full-span">
        ...
        <div class="button-row">
          <button class="btn btn-primary" data-admin-login-submit="true">Open Admin</button>
          <button class="btn btn-ghost" data-modal-close="true">Cancel</button>
        </div>`
    },
    cart: {
  ...
  function openModal(type) {
    ...
    modalBody.querySelectorAll('[data-lead-submit]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        const validationMessage = validateLeadForm(leadType, modalBody);
    ...
    modalBody.querySelectorAll('[data-admin-login-submit]').forEach((button) => {
      button.addEventListener('click', () => {
        const passwordInput = modalBody.querySelector('#admin-password');
        const enteredPassword = passwordInput?.value ?? '';

        if (String(enteredPassword).trim() !== 'admin123') {
          setToast('Incorrect password. Access denied.');
          return;
        }
  ...
  if (bestSellersTrigger) {
    bestSellersTrigger.addEventListener('click', (event) => {
      event.preventDefault();
      if (modal) closeModal();
